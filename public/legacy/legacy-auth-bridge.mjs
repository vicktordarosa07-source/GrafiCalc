...
const legacyUser = {
  ...
  status: "active", groupId: user.papel === "admin" ? "developer" : user.papel === "funcionario" ? "funcionarios" : "profissional",
  ...
  createdAt: user.criado_em || now, updatedAt: now,
};
localStorage.setItem("graficalc-auth-users-v1", JSON.stringify([legacyUser]));
localStorage.setItem("graficalc-auth-session-v1", JSON.stringify({ userId: user.id, username: legacyUser.username, role: legacyUser.role, loggedAt: now }));
if (legacyUser.role === "developer") sessionStorage.setItem("graficalc-config-unlocked-v1", "true");
